"""
Risk Assessment API Routes - FIXED for dict user
Handles both User object and dict from get_current_user
"""
...
# Helper function to get user_id from either dict or User object
def get_user_id(current_user: Union[User, dict]) -> str:
...
async def calculate_risk(
    ...
    try:
        ...
        result = service.assess_user_risk(user_id)
        return result
...
async def get_latest_risk(
    ...
    try:
        service = RiskAssessmentService(db)
        user_id = get_user_id(current_user)
        history = service.get_risk_history(user_id, limit=1)
...
async def get_risk_history(
    ...
    try:
        service = RiskAssessmentService(db)
        user_id = get_user_id(current_user)
        history = service.get_risk_history(user_id, limit=limit)
        return history
    except Exception as e:
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def get_recommendations(
    ...
    try:
        ...
        age = service.calculate_age(user.date_of_birth)
...
async def get_risk_dashboard(
    ...
    try:
        ...
        # Get latest assessment
        latest_list = service.get_risk_history(user_id, limit=1)
        latest = latest_list[0] if latest_list else None
        
        # Get history for trend
        history = service.get_risk_history(user_id, limit=6)
